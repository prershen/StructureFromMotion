...
def draw_correspondences(img, ptsTrue, ptsReproj, ax, drawOnly=50): 
    ...
    random_idx = np.random.choice(ptsTrue.shape[0],size=(drawOnly,),replace=False)
    true_pts, reprojected_pts = ptsTrue[random_idx], ptsReproj[random_idx]
    ax.scatter(true_pts[:,0],true_pts[:,1],marker='x',c='r',linewidths=.1, label='Ground Truth')
    ax.scatter(reprojected_pts[:,0],reprojected_pts[:,1],marker='x',c='b',linewidths=.1, label='Reprojected')
    ax.legend()
