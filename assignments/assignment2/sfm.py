...
def set_arguments(parser): 
    ...
    parser.add_argument('--dataset',action='store',type=str,default='teddy',dest='dataset',
    ...
    parser.add_argument('--outlier_thres',action='store',type=float,default=0.9,
...
class SFM(object): 
        ...
        for prev_name in self.image_data.keys(): 
            if prev_name != name: 
                ...
                if len(matches) > 0: 
                    ...
                    img1_points, img2_points, img1_idx, img2_idx = self.get_aligned_matches(kp1,desc1,kp2,
                                                                                desc2,matches)
                    
                    F,mask = cv2.findFundamentalMat(img1_points,img2_points,opts.fund_method,
                                                    opts.outlier_thres,
                                                    opts.fund_prob)
                    mask = mask.astype(bool).flatten()

                    self.matches_data[(prev_name,name)] = [matches, img1_points[mask], img2_points[mask], 
                                                img1_idx[mask],img2_idx[mask]]
    ...
    def compute_reprojection_error(self, name): 
        ...
        def compute_reprojections(X,R,t,K): 
        ...
        kp, desc = self.load_features(name)
        err = 0
        reproj_pts = compute_reprojections(self.point_cloud[ref[ref>0].astype(int)], R, t, self.K)
